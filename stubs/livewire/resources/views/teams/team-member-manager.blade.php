                                        ...
                                        <div class="{{ isset($addTeamMemberForm['role']) && $addTeamMemberForm['role'] !== $role->key ? 'opacity-50' : '' }}">
                                            ...
                                            <div class="flex items-center">
                                                <div class="text-sm text-gray-600 {{ $addTeamMemberForm['role'] == $role->key ? 'font-semibold' : '' }}">
                                                    {{ $role->name }}
                                                    <!-- Role Default? -->
                                                    @if(preg_match('(topmanager|comanager|manager|supervisor|client|worker)', $role->key) === 1)
                                                        <span class="px-4 py-2 text-xs bg-indigo-100 text-red-600 rounded-full shadow-sm">
                                                            {{ __('Default Role') }}
                                                        </span>
                                                    @endif
                                                </div>
                        ...
                        @foreach ($team->users->sortBy('name') as $user)
                            <div class="flex items-center justify-between">
                                ...
                                <div class="flex items-center">
                                    ...
                                    @if (Gate::check('updateTeamMember', $team) && Custura\Trane\Trane::hasRoles())
                                        <button class="ml-2 text-sm text-gray-400 underline" wire:click="manageRole('{{ $user->id }}')">
                                            {{ Custura\Trane\Trane::findRole($user->membership->role)->name }}
                                        </button>
                                    @elseif (Custura\Trane\Trane::hasRoles())
